...
public class CWPlaced : MonoBehaviour
{
    ...
    public CWDragAndDrop orig_clock;
            ...
            if (Input.touchCount > 0)
            {
                ...
                if (touch.phase == TouchPhase.Began)
                {
                    ...
                    if (collide_obj == Physics2D.OverlapPoint(_2D))     //check if the beginning touch point is within the currently placed token
                    {
                        ...
                        obj.GetComponent<RotateCW90>().rotate_active = false;
                    }
    ...
    private IEnumerator touchEnd()
    {
        ...
        pick_obj.clockToken = false;    //clockwise rotation token no longer being moved         
        if (pick_obj.placed_obj)
        {
            ...
            if (pick_obj.trash)
            {
                ...
                orig_clock.clock_token_remain++;        //if obj was placed in trash increment token counter
            }
        }
        else
        {
            obj.GetComponent<RotateCW90>().rotate_active = true;
            obj.transform.localPosition = prev_loc;
        }
    }
}
