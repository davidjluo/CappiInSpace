...
public class HealthPowerupDrag : MonoBehaviour {
	...
	void Update () {
        if (health_token_remain > 0 && (pick_obj.game_run == false) && pick_obj.health_token_used == false)
        {
    ...
    private IEnumerator touchEnd()
    {
        ...
        if (pick_obj.placed_obj)        //if placed in empty square decrement token counter
        {
            health_token_remain--;
            pick_obj.placed_obj = false;
            pick_obj.health_token_used = true;
        }
        objPicked = false;
        pick_obj.healthToken = false;
